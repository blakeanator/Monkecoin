...
dnl Output: If qt version is auto, set monkecoin_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$monkecoin_qt_want_version" = xauto && test "x$monkecoin_qt_force" != xyes; then
    if test "x$monkecoin_enable_qt" != xno; then
      AC_MSG_WARN([$1; monkecoin-qt frontend will not be built])
    fi
    monkecoin_enable_qt=no
    monkecoin_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "x$monkecoin_enable_qt" != xno && test "x$monkecoin_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build monkecoin-qt GUI (default=auto)])],
    [
     monkecoin_qt_want_version=$withval
     if test "x$monkecoin_qt_want_version" = xyes; then
       monkecoin_qt_force=yes
       monkecoin_qt_want_version=auto
     fi
    ],
    [monkecoin_qt_want_version=auto])
...
dnl Outputs: monkecoin_enable_qt, monkecoin_enable_qt_dbus, monkecoin_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "x$monkecoin_cv_static_qt" = xyes; then
  ...
  BITCOIN_QT_CHECK([
    monkecoin_enable_qt=yes
    monkecoin_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      monkecoin_enable_qt_test=no
    fi
    monkecoin_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      monkecoin_enable_qt_dbus=yes
    fi
  ...
  if test x$monkecoin_enable_qt = xyes; then
    AC_MSG_RESULT([$monkecoin_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$monkecoin_enable_qt])
  fi
...
dnl Output: monkecoin_cv_static_qt=yes|no
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, monkecoin_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [monkecoin_cv_static_qt=yes],
      [monkecoin_cv_static_qt=no])
    ])
])
