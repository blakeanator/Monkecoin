...
export P_CI_DIR="${BASE_BUILD_DIR}/monkecoin-$HOST"
...
if [[ ${USE_MEMORY_SANITIZER} == "true" ]]; then
  ...
  # details.
  DOCKER_EXEC 'grep -v HAVE_SYS_GETRANDOM src/config/monkecoin-config.h > src/config/monkecoin-config.h.tmp && mv src/config/monkecoin-config.h.tmp src/config/monkecoin-config.h'
fi
