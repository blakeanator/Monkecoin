Bitcoin Core version 0.13.1 is now available from:
  ...
  <https://monkecoin.org/bin/monkecoin-core-0.13.1/>
...
updates will be released anymore. Without security updates, using a monkecoin
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Monkecoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Monkecoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Monkecoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of monkecoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of monkecoins being spent is exactly
...
- **Increased security for multisig:** Monkecoin addresses (both P2PKH addresses
  ...
  believed to be provided by Monkecoin's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  Monkecoin paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of Monkecoin's current ledger (called the UTXO set),
...
- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Monkecoin users to individually opt-in to almost any change in the Monkecoin
  ...
  Other Monkecoin community members are studying several other improvements
...
[Segwit FAQ]: https://monkecoincore.org/en/2016/01/26/segwit-benefits/
[segwit wallet developers guide]: https://monkecoincore.org/en/segwit_wallet_dev/
[BIP141]: https://github.com/monkecoin/bips/blob/master/bip-0141.mediawiki
[BIP143]: https://github.com/monkecoin/bips/blob/master/bip-0143.mediawiki
[BIP144]: https://github.com/monkecoin/bips/blob/master/bip-0144.mediawiki
[BIP145]: https://github.com/monkecoin/bips/blob/master/bip-0145.mediawiki
[versionbits FAQ]: https://monkecoincore.org/en/2016/06/08/version-bits-miners-faq/
...
[BIP147]: https://github.com/monkecoin/bips/blob/master/bip-0147.mediawiki
...
- `monkecoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `monkecoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
...
- #8848 `7a34a46` Add NULLDUMMY verify flag in monkecoinconsensus.h (jl2012)
...
- #8940 `5b4192b` Add x9 service bit support to dnsseed.bluematt.me, seed.monkecoinstats.com (TheBlueMatt, cdecker)
- #8944 `685e4c7` Remove bogus assert on number of oubound connections. (TheBlueMatt)
- #8949 `0dbc48a` Be more agressive in getting connections to peers with relevant services (gmaxwell)

### Build system
- #8293 `fa5b249` Allow building libmonkecoinconsensus without any univalue (luke-jr)
- #8492 `8b0bdd3` Allow building bench_monkecoin by itself (luke-jr)
...
- #8829 `2a8bca4` Add monkecoin-tx JSON tests (jnewbery)
...
- #8860 `0bee740` util: Move wait_monkecoinds() into stop_nodes() (MarcoFalke)
...
- #8683 `8a7d7ff` Fix incorrect file name bitcoin.qrc  (monkecoinsSG)
...
- #8817 `bcf3806` update monkecoin-tx to output witness data (jnewbery)
