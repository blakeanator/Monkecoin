Bitcoin Core version *0.15.0* is now available from:
  ...
  <https://monkecoin.org/bin/monkecoin-core-0.15.0/>
...
Multi-wallet is enabled by using more than one `-wallet` argument when starting Monkecoin, either on the command line or in the Monkecoin config file.
...
Bitcoin Core 0.15.0 contains the following changes to the RPC interface and `monkecoin-cli` for multi-wallet:

* When running Bitcoin Core with a single wallet, there are **no** changes to the RPC interface or `monkecoin-cli`. All RPC calls and `monkecoin-cli` commands continue to work as before.
* When running Bitcoin Core with multi-wallet, all *node-level* RPC methods continue to work as before. HTTP RPC requests should be send to the normal `<RPC IP address>:<RPC port>` endpoint, and `monkecoin-cli` commands should be run as before. A *node-level* RPC method is any method which does not require access to the wallet.
* When running Bitcoin Core with multi-wallet, *wallet-level* RPC methods must specify the wallet for which they're intended in every request. HTTP RPC requests should be send to the `<RPC IP address>:<RPC port>/wallet/<wallet name>` endpoint, for example `127.0.0.1:8332/wallet/wallet1.dat`. `monkecoin-cli` commands should be run with a `-rpcwallet` option, for example `monkecoin-cli -rpcwallet=wallet1.dat getbalance`.
...
- `uptime` returns the total runtime of the `monkecoind` server since its last start (See [PR 10400](https://github.com/bitcoin/bitcoin/pull/10400)).
...
- Error codes have been updated to be more accurate for the following error cases (See [PR 9853](https://github.com/bitcoin/bitcoin/pull/9853)):
  ...
  - `removeprunedfunds` now returns RPC_WALLET_ERROR if `monkecoind` is unable to remove
  the transaction. Previously returned RPC_INTERNAL_ERROR.
  - `removeprunedfunds` now returns RPC_INVALID_PARAMETER if the transaction does not
  exist in the wallet. Previously returned RPC_INTERNAL_ERROR.
  - `fundrawtransaction` now returns RPC_INVALID_ADDRESS_OR_KEY if an invalid change
  address is provided. Previously returned RPC_INVALID_PARAMETER.
  - `fundrawtransaction` now returns RPC_WALLET_ERROR if `monkecoind` is unable to create
...
- #10475 `39039b1` [RPC] getmempoolinfo mempoolminfee is a MKE/KB feerate (instagibbs)
...
- #9672 `46311e7` Opt-into-RBF for RPC & monkecoin-tx (luke-jr)
...
- #10400 `1680ee0` [RPC] Add an uptime command that displays the amount of time (in seconds) monkecoind has been running (rvelhote)
- #10683 `d81bec7` rpc: Move the `generate` RPC call to rpcwallet (laanwj)
- #10710 `30bc0f6` REST/RPC example update (Mirobit)
- #10747 `9edda0c` [rpc] fix verbose argument for getblock in monkecoin-cli (jnewbery)
...
- #10228 `27faa6c` build: regenerate monkecoin-config.h as necessary (theuni)
...
- #10017 `02d64bd` combine_logs.py - aggregates log files from multiple monkecoinds during functional tests (jnewbery)
...
- #10142 `ed09dd3` Run monkecoin_test-qt under minimal QPA platform (ryanofsky)
...
- #10359 `329fc1d` [tests] functional tests should call MonkecoinTestFramework start/stop node methods (jnewbery)
...
- #10636 `4bc853b` [qa] util: Check return code after closing monkecoind proc (MarcoFalke)
...
- #10556 `65cc7aa` Move stop/start functions from utils.py into MonkecoinTestFramework (jnewbery)
...
- #10090 `8e4f7e7` Update monkecoin.conf with example for pruning (coinables)
...
- #10447 `f259263` Make monkecoind invalid argument error message specific (laanwj)
- #10495 `6a38b79` contrib: Update location of seeds.txt (laanwj)
- #10469 `b6b150b` Fixing typo in rpcdump.cpp help message (keystrike)
- #10451 `27b9931` contrib/init/monkecoind.openrcconf: Don't disable wallet by default (luke-jr)
...
- #10651 `cef4b5c` Verify binaries from monkecoincore.org and monkecoin.org (TheBlueMatt)
- #10786 `ca4c545` Add PR description to merge commit in github-merge.py (sipa)
- #10812 `c5904e8` [utils] Allow monkecoin-cli's -rpcconnect option to be used with square brackets (jnewbery)
...
- #9999 `a328904` [LevelDB] Plug leveldb logs to monkecoin logs (NicolasDorier)
...
- #10958 `659c096` Update to latest Monkecoin patches for LevelDB (sipa)
