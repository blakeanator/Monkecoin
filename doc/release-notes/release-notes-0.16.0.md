Bitcoin Core version 0.16.0 is now available from:
  ...
  <https://monkecoincore.org/bin/monkecoin-core-0.16.0/>
...
monkecoin data directory. The behavior is now:
...
GUI changes
-----------
- Uses of "µMKE" in the GUI now also show the more colloquial term "bits", specified in BIP176.
...
- monkecoin-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- monkecoin-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.

Testing changes
----------------
- The default regtest JSON-RPC port has been changed to 18443 to avoid conflict with testnet's default of 18332.
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest monkecoin.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.monkecoin.sprovoost.nl (Sjors)
...
- #11970 `b7450cd` Add test coverage for monkecoin-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to monkecoin-cli to allow RPC password to be read from standard input (jharvell)
...
- #12035 `eeb6d52` Change µMKE to bits (jb55)
...
- #11711 `6378e5c` monkecoin_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.monkecoin doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for monkecoin-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in monkecoin-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop monkecoind (OmeGak)
...
- #10529 `331352f` Improve monkecoind systemd service file (Flowdalic)
