Bitcoin Core version 0.18.0 is now available from:
  ...
  <https://monkecoincore.org/bin/monkecoin-core-0.18.0/>
...
`/Applications/Monkecoin-Qt` (on Mac) or `monkecoind`/`monkecoin-qt` (on
Linux).
...
`contrib/init/monkecoind.service`, it has been changed to use
`/var/lib/monkecoind` as the data directory instead of
`~monkecoin/.monkecoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/monkecoind` will
exist has enough space (check using `df -h /var/lib/monkecoind`), and
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://monkecoincore.org/en/contact/) the Bitcoin Core
...
The systemd init file (`contrib/init/monkecoind.service`) has been changed
to use `/var/lib/monkecoind` as the data directory instead of
`~monkecoin/.monkecoin`. This change makes Bitcoin Core more consistent with
...
- A new [document](https://github.com/bitcoin/bitcoin/blob/master/doc/monkecoin-conf.md)
  about the `monkecoin.conf` file describes how to use it to configure
  Bitcoin Core.

- A new document introduces Bitcoin Core's BIP174 [Partially-Signed
  Monkecoin Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Monkecoin-Qt from being built with support for the BIP70 payment
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart monkecoind with the `-deprecatedrpc=generate`
...
- A new `monkecoin-wallet` tool is now distributed alongside Monkecoin
...
other Monkecoin software and services.
...
- A sub-project of Bitcoin Core now provides Hardware Wallet Interaction
  ...
  page](https://github.com/monkecoin-core/HWI#readme) for details.
...
- #12490 Remove deprecated wallet rpc features from `monkecoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libmonkecoin_server.a` (jnewbery)
...
- #13926 [Tools] monkecoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like monkecoind (MarcoFalke)
...
- #14521 qt, docs: Fix `monkecoin-qt -version` output formatting (hebasto)
...
- #11625 Add MonkecoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libmonkecoin_server` (laanwj)
...
- #15154 Configure: monkecoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/monkecoin-wallet (Sjors)
...
- #15110 build\_msvc: Fix the build problem in `libmonkecoin_server` (Mr-Leshiy)
...
- #12246 Bugfix: Only run monkecoin-tx tests when monkecoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_monkecoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_monkecoin` (MarcoFalke)
...
- #14186 monkecoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_monkecoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update monkecoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/monkecoin-conf.md (hebasto)
...
- #15354 Add missing `monkecoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct monkecoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support monkecoin folder on OSX (JimmyMow)
