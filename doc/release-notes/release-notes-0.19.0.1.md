Bitcoin Core version 0.19.0.1 is now available from:
  ...
  <https://monkecoincore.org/bin/monkecoin-core-0.19.0.1/>
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Monkecoin
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `monkecoind -help` output for these two options for
...
- The `testnet` field in `monkecoin-cli -getinfo` has been renamed to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Monkecoin address without
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  MKE), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in MKE per 1,000
...
- #15649 Add ChaCha20Poly1305@Monkecoin AEAD (jonasschnelli)
...
- #15639 monkecoin-wallet tool: Drop libmonkecoin_server.a dependency (ryanofsky)
...
- #16521 Use the default maxfeerate value as MKE/kB (Remagpie)
...
- #16366 init: Use InitError for all errors in monkecoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "monkecoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of monkecoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from monkecoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_monkecoin to auto generated project files (sipsorcery)
...
- #15773 Add MonkecoinTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_monkecoin output (practicalswift)
...
- #15896 feature_filelock, interface_monkecoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Bitcoin Core (luke-jr)
...
- #16277 Suppress output in test_monkecoin for expected errors (gertjaap)
...
- #16929 follow-up to rpc: default maxfeerate value as MKE/kB (jonatack)
...
- #16489 log: harmonize monkecoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (monkecoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example monkecoin.conf (fanquake)
...
- #16080 build/doc: update monkecoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in monkecoin.conf (fanquake)
...
- #16621 add default monkecoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the monkecoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, monkecoin.conf, and Info.plist.in (ch4ot1c)
