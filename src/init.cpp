 ...
 */
static const char* BITCOIN_PID_FILENAME = "monkecoind.pid";
...
/** Sanity checks
 *  Ensure that Monkecoin is running in a usable environment with all
...
{
    // Make sure only a single Monkecoin process is using the data directory.
...
bool AppInitMain(const util::Ref& context, NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !fs::path(args.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if monkecoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if monkecoin is started while in a temporary directory.\n",
