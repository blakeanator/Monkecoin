...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<MonkecoinAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_monkecoin-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_monkecoin-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_monkecoin-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = MonkecoinUnits::formatWithUnit(unit, balance, false, MonkecoinUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    MonkecoinAmountField* amountInput = receiveCoinsDialog.findChild<MonkecoinAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(uri.count("monkecoin:"), 2);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_monkecoin-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
