...
const char * const BITCOIN_CONF_FILENAME = "monkecoin.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //monkecoin-tx using stdin
        std::string val;
...
#else
    const char* pszModule = "monkecoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Monkecoin
    // macOS: ~/Library/Application Support/Monkecoin
    // Unix-like: ~/.monkecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Monkecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Monkecoin";
#else
    // Unix-like
    return pathRet / ".monkecoin";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Monkecoin Core copyright is not removed by accident
    if (copyright_devs.find("Monkecoin Core") == std::string::npos) {
