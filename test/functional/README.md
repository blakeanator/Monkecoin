...
- Use a module-level docstring to describe what the test is testing, and how it
  is testing it.
- When subclassing the MonkecoinTestFramework, place overrides for the
...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to control whether
  ...
  has 25 mature blocks (25x50=1250 MKE) in its wallet.
...
with the monkecoind(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a monkecoind.  `P2PInterface`
...
The [`TestShell`](test-shell.md) class exposes the MonkecoinTestFramework
...
Taken from the [python-monkecoinrpc repository](https://github.com/jgarzik/python-monkecoinrpc).
...
Test objects for interacting with a monkecoind node over the p2p interface.

#### [script.py](test_framework/script.py)
Utilities for manipulating transaction scripts (originally from python-monkecoinlib)
