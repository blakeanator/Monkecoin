...
class BackwardsCompatibilityTest(MonkecoinTestFramework):
    ...
    def run_test(self):
        ...
        # RPC loadwallet failure causes monkecoind to exit, in addition to the RPC
        # call failure, so the following test won't work:
        # assert_raises_rpc_error(-4, "Wallet loading failed.", node_v17.loadwallet, 'w3_v18')

        # Instead, we stop node and try to launch it with the wallet:
        self.stop_node(4)
        node_v17.assert_start_raises_init_error(["-wallet=w3_v18"], "Error: Error loading w3_v18: Wallet requires newer version of Monkecoin Core")
        if self.options.descriptors:
            ...
            node_v17.assert_start_raises_init_error(["-wallet=w3"], "Error: Error loading w3: Wallet requires newer version of Monkecoin Core")
        self.start_node(4)
        ...
        # Copy the 0.16 wallet to the last Monkecoin Core version and open it:
        shutil.copyfile(
        ...
        # Copy the 0.17 wallet to the last Monkecoin Core version and open it:
        ...
        # Copy the 0.19 wallet to the last Monkecoin Core version and open it:
        shutil.copytree(
