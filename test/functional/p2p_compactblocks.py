...
# TestP2PConn: A peer we use to send messages to monkecoind, and store responses.
...
class CompactBlocksTest(MonkecoinTestFramework):
    ...
    # This test actually causes monkecoind to (reasonably!) disconnect us, so do this last.
    ...
    # Test that monkecoind requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for monkecoind to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        version = test_node.cmpct_version
        node = self.nodes[0]
        # monkecoind will not send blocktxn responses for blocks whose height is
